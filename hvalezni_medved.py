...
def najdi_besede(besedilo, podniz):
    p = re.compile(r'\b(\w*{}\w*)\b'.format(podniz))
    t = p.findall(besedilo)
    return set(t)
...
def najdi_predpono(besedilo, predpona):
    p = re.compile(r'\b({}\w*)\b'.format(predpona))
    t = p.findall(besedilo)
    return set(t)
...
def najdi_pripono(besedilo, predpona):
    p = re.compile(r'(\w*{})\b'.format(predpona))
    t = p.findall(besedilo)
    return set(t)
...
def podvojene_crke(besedilo):
    p = re.compile(r'\b(\w*(\w)\2\w*)\b')
    t = p.findall(besedilo)
    return [a[0] for a in t]
